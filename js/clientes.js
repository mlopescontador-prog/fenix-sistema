...
function abrFichaCli(cid){
  ...
  if(ctr){
    ...
    // Botões
    document.getElementById('fc-ctrBtns').innerHTML=
      `<button class="btn bo bsm" onclick="closeM('mFichaCli');abrFichaContr(${ctr.id})">📋 Ver contrato</button>`+
      ((ctr.status||'ATIVO')!=='ENCERRADO'?`<button class="btn bo bsm" style="color:var(--acc);border-color:rgba(5,150,105,.3)" onclick="closeM('mFichaCli');abrRenov(${ctr.id})">🔄 Renovar</button>`:'');
    fcEl.style.display='';
  } else {
