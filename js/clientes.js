...
function abrFichaCli(cid){
  ...
  // ── Seção Contrato ──────────────────────────────────────────────────
  const ctr=contr.filter(ct=>ct.cliente_id===cid).sort((a,b)=>{
    const ord={ATIVO:0,SUSPENSO:1,INTERNO:2,ENCERRADO:3};
    return (ord[a.status||'ATIVO']||0)-(ord[b.status||'ATIVO']||0);
  })[0];
  const fcEl=document.getElementById('fc-contrato');
  if(ctr){
    // Badge status
    const stCor={ATIVO:'bg2',SUSPENSO:'by2',INTERNO:'bb2',ENCERRADO:'bk2'}[ctr.status||'ATIVO']||'bg2';
    document.getElementById('fc-ctrSt').innerHTML=`<span class="bdg ${stCor}" style="font-size:9px;">${ctr.status||'ATIVO'}</span>`;
    // Tributação
    document.getElementById('fc-ctrTrib').textContent=ctr.tributacao||'—';
    // Renovação + alerta
    const hoje=new Date();hoje.setHours(0,0,0,0);
    let renovTxt='—'; let renovCor='var(--txt)';
    if(ctr.renovacao){
      const dtR=new Date(ctr.renovacao+'T00:00:00');
      const diff=Math.round((dtR-hoje)/(86400000));
      renovTxt=dtR.toLocaleDateString('pt-BR',{month:'2-digit',year:'numeric'});
      renovCor=diff<0?'var(--red)':diff<=30?'var(--yellow)':'var(--muted2)';
      if(diff<0)renovTxt+=' ⚠ '+Math.abs(diff)+'d vencido';
      else if(diff<=30)renovTxt+=' ('+diff+'d)';
    }
    document.getElementById('fc-ctrRenov').textContent=renovTxt;
    document.getElementById('fc-ctrRenov').style.color=renovCor;
    // Tempo de cliente
    document.getElementById('fc-ctrTempo').textContent=ctr.inicio?calcTempo(ctr.inicio):'—';
    // Botões
    const btnsEl=document.getElementById('fc-ctrBtns');
    btnsEl.innerHTML='';
    const btnVer=document.createElement('button');
    btnVer.className='btn bo bsm';
    btnVer.innerHTML='📋 Ver contrato';
    btnVer.onclick=()=>{closeM('mFichaCli');setTimeout(()=>abrFichaContr(ctr.id),80);};
    btnsEl.appendChild(btnVer);
    if((ctr.status||'ATIVO')!=='ENCERRADO'){
      const btnRen=document.createElement('button');
      btnRen.className='btn ba bsm';
      btnRen.innerHTML='🔄 Renovar';
      btnRen.style.cssText='margin-left:5px;';
      btnRen.onclick=()=>{closeM('mFichaCli');setTimeout(()=>abrRenov(ctr.id),80);};
      btnsEl.appendChild(btnRen);
    }
    fcEl.style.display='';
  } else {
    fcEl.innerHTML=`<div style="display:flex;align-items:center;justify-content:space-between;"><span style="font-size:11px;color:var(--muted);">Sem contrato cadastrado</span><button class="btn bo bsm" onclick="closeM('mFichaCli');openM('mContr')">+ Cadastrar contrato</button></div>`;
    fcEl.style.display='';
  }
  openM('mFichaCli');
}
